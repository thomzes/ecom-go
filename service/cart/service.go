...
func checkIfCartInStock(cartItems []types.CartCheckoutItem, products map[int]types.Product) error {
	if len(cartItems) == 0 {
...
func (h *Handler) createOrder(products []types.Product, cartItems []types.CartCheckoutItem, userID int) (int, float64, error) {
	// create a map products for easier access
	productsMap := make(map[int]types.Product)
	for _, product := range products {
		productsMap[product.ID] = product
	}

	// check if product are available
	if err := checkIfCartInStock(cartItems, productsMap); err != nil {
		return 0, 0, err
	}
	...
	// reduce quantity of product in the store
	for _, item := range cartItems {
	...
	orderID, err := h.orderStore.CreateOrder(types.Order{
		...
		Total:   totalPrice,
		Status:  "pending",
		Address: "some address", // hard coded
	})
	if err != nil {
		return 0, 0, nil
	}
